...
```{r pca, fig.height=5, fig.width=8, include=FALSE, results='hide'}
...
```{r parallel-analysis, fig.height=5, fig.width=8, message=FALSE, warning=FALSE, include=FALSE, results='hide'}
# 4
...
```{r biplots, fig.height=5, fig.width=7, message=FALSE, warning=FALSE, include=FALSE, results='hide'}
...
```{r fig.cap= 'Scatter Correlations', fig.width=15, fig.height=15, dpi=600}
p
```

```{r fig.cap="Pearson Correlation"}
# Plot
ggplot(corr_df, aes(x = label, y = corr, fill = corr > 0)) +
  geom_col(width = 0.75) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.7) +
...
par(op)
```
```{r fig.cap="Permutation Test"}
perm_range <- permtestPCA(X_train)
```

```{r fig.cap=' Biplot Components 1-2'}
p12
```

```{r fig.cap='Biplot all Components'}
p12 + p23
```

```{r fig.cap= 'Biplot of the first two components', fig.width= 20, dpi= 300, fig.height=20}
biplot(pca_model, scale = 0)

```
```{r fig.cap= 'Optimal Visualization'}
fviz_pca_biplot(pca_model)
```

```{r fig.cap='Plot density + CI + point estimate'}
ggplot(df_boot, aes(x = vaf)) +
  geom_histogram(aes(y = ..density..),
                 bins = 40, fill = "skyblue3", color = "white", alpha = 0.6) +
  geom_density(color = "darkblue", linewidth = 1) +
...
```{r fig.cap= 'Boostrap Distribution'}
hist(vaf_boot,
     breaks = 30,
     main = sprintf("Bootstrap Distribution of VAF for First %d PCs", k),
     xlab = "VAF (Variance Accounted For)",
...
```{r fig.cap= 'RMSE by temperature decile' }
ggplot(rmse_long, aes(x = group, y = RMSE, color = Model, group = Model)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  theme_classic() +
